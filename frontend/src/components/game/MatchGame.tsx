...
import { Clock3, RotateCcw, ShieldAlert, Sparkles } from "lucide-react";
...
export default function MatchGame({ cards }: MatchGameProps) {
	...
	const elapsedTime =
		startAt === null ? 0 : (endAt ?? now) - startAt + penaltyTime;
	...
	return (
		<section className="space-y-3 pb-10 md:space-y-4 md:pb-14">
			...
			{isFinished && (
				...
				{`완료했습니다. 최종 기록은 ${formatSeconds(elapsedSeconds)}입니다.`}
				</div>
			)}
